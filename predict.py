...
def load_checkpoint(checkpoint_path):
   ...
   # network model architecture
    if state['arch']== 'densenet121':
        ...
        raise ValueError('Unkown network architecture', state['arch'])
    ...
    features.extend([
        nn.Dropout(),
        nn.Linear(in_feature, state['hidden_units']),
        nn.ReLU(True),
        nn.Dropout(),
        nn.Linear( state['hidden_units'], state['hidden_units']),
        nn.ReLU(True),
        nn.Linear( state['hidden_units'], 102)
    ])
